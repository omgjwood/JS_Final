...
$(document).ready(function(){
    ...
    function updateCart() {
        ...
        var itemQty = document.querySelectorAll("#checkoutcart .product-quantity");
        ...
        for (var i = 0; i < itemQty.length; i++) {
            ...
            pbh_cart.products.forEach(function(p) {
                ...
                //console.log(itemQty.getAttribute('data-id'));
                console.log(itemQty[0].value);
                console.log(itemQty[0].getAttribute('data-id'));

                if ( p.itemProductID == itemQty[0].getAttribute('data-id')) {
                    p.itemQty = itemQty[0].getAttribute('data-id');
                }
    ...
    function buildCart() {
        ...
        pbh_cart.products.forEach(function(p) {
            ...
            var html = [
                ...
                '</td>',
                '<td>',
                '<input value="' + p.itemQty + '" class="form-control product-quantity" type="number" data-id="' + p.itemProductID + '">',
                '</td>',
                '<td>$' + p.itemPrice + '</td>',
                '<td>$0.00</td>',
